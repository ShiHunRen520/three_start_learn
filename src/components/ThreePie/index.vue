<template>
  <main class="three-pie">
    <div class="three-pie-container" ref="chart"></div>
  </main>
</template>

<script>
import { initPie } from './index';
export default {
  name: "cityGreenLand",
  components: {},
  data () {
  ...
  methods: {
    ...
    getPie3D (pieData, internalDiameterRatio) {
      ...
      pieData.sort((a, b) => {  // 饼图数据排序
        return (b.value - a.value);
      });
      // 为每一个饼图数据，生成一个 series-surface 配置
      for (let i = 0; i < pieData.length; i++) {
        sumValue += pieData[i].value; // 收集pie数据之和
        let seriesItem = {
          name: typeof pieData[i].name === 'undefined' ? `series${i}` : pieData[i].name,
          type: 'surface',  // 类型 surface 表面
          parametric: true, // 参数
          wireframe: {  // 线框
            show: true
          },
          pieData: pieData[i],  // pie 表面的数据
          pieStatus: {  // pie 表面的状态
            selected: false,
            hovered: false,
            k: k
        ...
        if (typeof pieData[i].itemStyle != 'undefined') { // 为 pie 的设置颜色主题
          let itemStyle = {};
      ...
      for (let i = 0; i < series.length; i++) {
        endValue = startValue + series[i].pieData.value;  // 计算出每一个series类目的数据
        series[i].pieData.startRatio = startValue / sumValue; // 计算出每一个series类目的数据的开始角度
        series[i].pieData.endRatio = endValue / sumValue; // 每一个pie 的series 的结束角度
        series[i].parametricEquation = this.getParametricEquation(series[i].pieData.startRatio, series[i].pieData.endRatio,
          false, false, k, series[i].pieData.value);  // series的参数的方程
        startValue = endValue;  // 上一段扇形的结束值是本次的扇形的开始值
        let bfb = that.formatFloat(series[i].pieData.value / sumValue, 4); // 计算每一段扇形的百分比
        legendData.push({ // legend将保存一份数据，进行legend图例生成
          name: series[i].name,
          value: bfb
        });
        legendBfb.push({
      ...
      let option = {
        legend: {
          ...
          formatter: function (param) {
            ...
            let bfs = that.formatFloat(item.value * 100, 2) + "%";
    ...
    //获取3d pie 的最高扇区的高度 => 通过 传入的高度参数 * 25 / 最小数值的 pie的data
